...
def plot_overlap_J(overlap_J,L,N,beta,tot_steps):
    ...
    ax.plot(overlap_J[x+1][ex:-1],color_list[0],label="l=1")
    ax.plot(overlap_J[x+2][ex:-1],color_list[1],label="l=2")
    ax.plot(overlap_J[x+3][ex:-1],color_list[2],label="l=3")
    ax.plot(overlap_J[x+4][ex:-1],color_list[3],label="l=4")
    ax.plot(overlap_J[x+5][ex:-1],color_list[4],label='l=5')
    ax.plot(overlap_J[x+6][ex:-1],color_list[5],label='l=6')
    ax.plot(overlap_J[x+7][ex:-1],color_list[6],label='l=7')
    ax.plot(overlap_J[x+8][ex:-1],color_list[7],label="l=8")
    ax.plot(overlap_J[x+9][ex:-1],color_list[8],label="l=9")
    #plt.legend(loc="upper right")
    plt.legend(loc="lower left")
    plt.xlabel("t")
    plt.ylabel(r"$Q(t,l)$")
    ax.set_title("L={:d}; N={:d}; beta={:3.1f}".format(L,N,beta))
    plt.savefig("../imag/Overlap_new_J_L{:d}_N{:d}_beta{:2.0f}_step{:d}.eps".format(L,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Overlap_new_J_L{:d}_N{:d}_beta{:2.0f}_step{:d}.png".format(L,N,beta,tot_steps),format='png')
...
def plot_overlap_S(overlap_S,L,M,N,beta,tot_steps):
    ...
    color_list = get_N_HexCol(num_color)
    ax = fig.add_subplot(111) # add_subplot() adds an axes to a figure, it returns a (subclass of a) matplotlib.axes.Axes object.
    ax.plot(overlap_S[x+1][ex:-1],color_list[0],label="l=1")
    ax.plot(overlap_S[x+2][ex:-1],color_list[1],label="l=2")
    ax.plot(overlap_S[x+3][ex:-1],color_list[2],label="l=3")
    ax.plot(overlap_S[x+4][ex:-1],color_list[3],label="l=4")
    ax.plot(overlap_S[x+5][ex:-1],color_list[4],label='l=5')
    ax.plot(overlap_S[x+6][ex:-1],color_list[5],label='l=6')
    ax.plot(overlap_S[x+7][ex:-1],color_list[6],label='l=7')
    ax.plot(overlap_S[x+8][ex:-1],color_list[7],label="l=8")
    ax.plot(overlap_S[x+9][ex:-1],color_list[8],label="l=9")
    plt.legend(loc="upper right")
    plt.xlabel("t")
    plt.ylabel(r"$q(t,l)$")
    ax.set_title("L={:d}; M={:d}; N={:d}; beta={:3.1f}".format(L,M,N,beta))
    plt.savefig("../imag/Overlap_new_S_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}.eps".format(L,M,N,beta,tot_steps),format='eps')
    plt.savefig("../imag/Overlap_new_S_L{:d}_M{:d}_N{:d}_beta{:2.0f}_step{:d}.png".format(L,M,N,beta,tot_steps),format='png')
...
if __name__ == '__main__':
    ...
    for i in range(nk):
        ...
        beta = beta_tmp[0]
        J = np.load('{}/{}/J_new_host_L{:d}_N{:d}_beta{:3.1f}_step{:d}.npy'.format(data_dir,timestamp_list[i],L,N,beta,tot_steps))
        J0 = np.load('{}/{}/J_new_guest_L{:d}_N{:d}_beta{:3.1f}_step{:d}.npy'.format(data_dir,timestamp_list[i],L,N,beta,tot_steps))
        S = np.load('{}/{}/S_new_host_L{:d}_M{:d}_N{:d}_beta{:3.1f}_step{:d}.npy'.format(data_dir,timestamp_list[i],L,M,N,beta,tot_steps))
        S0 = np.load('{}/{}/S_new_guest_L{:d}_M{:d}_N{:d}_beta{:3.1f}_step{:d}.npy'.format(data_dir,timestamp_list[i],L,M,N,beta,tot_steps))
